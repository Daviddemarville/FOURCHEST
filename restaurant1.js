...
const restaurantTable = [
	{
		...
		city: "Nancy",
		img: "./public/images/resto1.jpg",
		imgNote: "./public/images/star.png",
		isVegetarian: false,
		isLocal: true,
		score: "4.7",
	},
...
function restaurantFilter(table) {
	...
	if (citySelector.value !== "Ville") {
		filteredTable = filteredTable.filter(
			(restaurant) => restaurant.city === citySelector.value,
		);
	}
	...
	if (buttonVege.checked) {
		filteredTable = filteredTable.filter(
			(restaurant) => restaurant.isVegetarian,
		);
	}
...
function cleanScreen() {
	...
	restoDelete.forEach((resto) => {
		resto.remove();
	}); // Supprime toutes les cartes affichées
}
